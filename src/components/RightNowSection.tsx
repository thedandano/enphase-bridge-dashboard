...
export function RightNowSection() {
  ...
  const dash = "—";
  const fmt = (wh: number) =>
    data === null || noData ? dash : data.is_complete ? toKw(wh) : toWh(wh);
  ...
  return (
    <div className={styles.section}>
      ...
      <div className={styles.grid}>
        <StatCard
          label="Producing"
          value={fmt(data?.wh_produced ?? 0)}
          color="var(--signal-production)"
          isInProgress={inProgress}
        />
        <StatCard
          label="Consuming"
          value={fmt(data?.wh_consumed ?? 0)}
          color="var(--signal-consumption)"
          isInProgress={inProgress}
        />
        <StatCard
          label="Exporting"
          value={fmt(data?.wh_grid_export ?? 0)}
          color="var(--signal-grid-export)"
          isInProgress={inProgress}
        />
        <StatCard
          label="Importing"
          value={fmt(data?.wh_grid_import ?? 0)}
          color="var(--signal-grid-import)"
          isInProgress={inProgress}
        />
      </div>
    </div>
